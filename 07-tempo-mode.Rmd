...
- More complex Brownian models. yes
- Non-Brownian trait evolution. yes
- The SLOUCH family of models yes
...
In addition, although OU models improve upon Brownian models by having traits evolve in more realistic directions, they suffer from the same problem that the rate of evolution, $\sigma^2$, is constant across the tree. We can however, improve on this by fitting OU models with different $\sigma^2$ values for each value of $\mu$. Take care here of overfitting (see *Chapter 3*)! Even a modest number of groups will increase the number of parameters you are fitting considerably, for example a single stationary peak model has 4 parameters ($z(0)$, $\sigma^2$, $\mu$ and $\alpha$); a multi peak model with three groups has $\sigma^2$, $\mu$ and $\alpha$ for each group.
