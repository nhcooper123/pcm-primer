...
Phylogenetic imbalance occurs where sister clades on a phylogeny have different numbers of species. Phylogenies can vary from completely imbalanced (Figure \@ref(fig:imbalance))  where one sister clade has one species and all the others are in the other clade, and fully balanced, where the two clades have an equal number of species (Figure \@ref(fig:imbalance)). Some degree of imbalance is expected in real phylogenies, so to measure how imbalanced a tree is we can compare it to a simulated trees with the same number of species.
...
# LTT plots
lt1 <- ltt(tree1, gamma = FALSE, plot = FALSE)
lt2 <- ltt(tree2, gamma = FALSE, plot = FALSE)
lt3 <- ltt(tree3, gamma = FALSE, plot = FALSE)

# Make dataframes
lt1 <- data.frame(times = c(1,2,4,6,8), ltt = c(2,3,8,16,32))
lt2 <- data.frame(times = lt2$times, ltt = lt2$ltt)
lt3 <- data.frame(times = lt3$times, ltt = lt3$ltt)

plotlt1 <-
  ggplot(lt1, aes(x = times, y = log(ltt))) +
  geom_line() +
...
plotlt2 <-
  ggplot(lt2, aes(x = times, y = log(ltt))) +
  geom_line() +
  theme_classic(base_size = 14) +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
plotlt3 <-
  ggplot(lt3, aes(x = times, y = log(ltt))) +
  geom_line() +
  theme_classic(base_size = 14) +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
...
In Figure \@ref(fig:ltt) we used simulated data to get our LTT plots, but in reality these plots are rarely quite so straightforward to interpret. Figure \@ref(fig:ltt-real) shows the real LTT plot for a phylogeny of natricine snakes [DEEPAK; see *online materials*), where it is much less obvious to tell what is going on than in our simulated data.

```{r ltt-real, echo = FALSE, fig.cap = 'YYY', out.width='80%', fig.asp=.75, fig.align = 'left'}
...
A large variety of methods have been developed that attempt to deal with this problem including MEDUSA [@alfaro2009nine], various methods in BayesTraits [@pagel2007bayestraits], BAMM [@rabosky2014automatic], and RevBayes [@hohna2016revebayes]. The method you are most likely to come across in the literature is BAMM, so we will briefly outline this here.
