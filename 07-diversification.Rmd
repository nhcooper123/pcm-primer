# A very brief introduction to diversification analyses {#diversification}
...
In this chapter we are going to very briefly introduce you to diversification analyses. Most introductory phylogenetic comparative methods courses cover this at least briefly, but a lot of the nuance lies beyond the scope of this book. If you need to use these kinds of methods for your analyses this should be a starting point for further reading and exploration. An excellent introduction can be found in [@harmonbook].

## Why do we study species diversity?
Diversity, or species richness, is not evenly spread across the Tree of Life. Some groups have lots of species, while others have very few. A classic example is that of the animal phyla, where one single phylum, Arthropoda, has over 1 million species, while other phyla, for example [Picozoa](https://en.wikipedia.org/wiki/Picozoa), have only one or just a few species. This uneven diversity is found at all taxonomic levels; whether we look at species in phyla, or genera in families etc., we tend to see the same pattern. Some groups are more diverse than others. This has led to many biologists over the years trying to find out why this pattern exists. Is there something special about groups with lots of species? 

Many questions, predictions and hypotheses exist around uneven diversity. These form some of the most interesting and long lasting ideas in evolutionary biology. Questions include things like...

- Are changes in diversification rates driven by abiotic factors like temperature?
- Have certain branches of a phylogenetic tree experienced significant shifts in diversification rates compared to others?
- Do variations in diversification rates explain the differences in species richness across the Tree of Life?
- Do specific traits or characters (e.g., body size, ecological niche) influence the speciation or extinction rates of lineages?
- Do diversification rates stay constant, or do they fluctuate over time (e.g. early bursts vs. late declines)?
...
```{r imbalance, echo = FALSE, message = FALSE, warning= FALSE, fig.cap = 'Imbalanced versus balanced phylogenetic trees.', out.width='80%', fig.asp=.75, fig.align = 'center'}
...
```{r ltt, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = 'Trees with constant, increasing and decreasing rates of speciation.', out.width='80%', fig.asp=.75, fig.align = 'center'}
...
t1 <- 
  ggtree(tree1) + 
  theme_tree() +
  labs(title= "Constant rate") +
  theme(plot.title = element_text(hjust = 0.5))

t2 <- 
  ggtree(tree2) + 
  theme_tree() +
  labs(title = "Increasing rate") +
  theme(plot.title = element_text(hjust = 0.5))

t3 <- 
  ggtree(tree3) + 
  theme_tree() +
  labs(title = "Decreasing rate") +
  theme(plot.title = element_text(hjust = 0.5))
...
### BAMM!
BAMM (Bayesian Analysis of Macroevolutionary Mixtures; [@rabosky2014automatic], [@rabosky2014bamm]) identifies discrete shifts in diversification rates at nodes of a tree (it can also investigate trait evolution but we will focus on diversification rates here), i.e places where diversification rates speed up or slow down. BAMM looks for rate shifts across the whole tree, so it can find one or more shifts. It does this using __reversible jump Markov Chain Monte Carlo (MCMC)__ methods to automatically explore a vast universe of possible models (reversible jump MCMC is a special kind of MCMC algorithm; see [@rabosky2014automatic] for details). It is biased towards simpler models (a common tactic in most evolutionary models, e.g. parsimony) so rarely results in lots of rate shifts. 

```{block, type = "info"}
<center>
### A quick (very simplified) recap of Bayesian models. 
</center>

Bayesian methods calculate the probability of the model given the data, *P(model|data)*, whereas Maximum Likelihood models calculate the probability of the data given the model, *P(data|model)*. 
...
This is a simplification of course! But the take home is that to fit Bayesian models we need to consider *priors*, *burnin*, *number of iterations*, *thinning*, *convergence* and the *posterior* (in addition to the model, data and phylogeny!). If you want to learn more about Bayesian statistics we highly recommend Ricahrd McElreath's book [@mcelreath2020statistical] and the accompanying [online course](https://www.youtube.com/watch?v=ztbYkBPDOgU)
```
...
css <- credibleShiftSet(edata, expectedNumberOfShifts = 1, 
                        threshold = 5, set.limit = 0.95, verbose = FALSE)
...
Additionally, there has been a quite a lot of debate about the validity of BAMM since 2016. Moore et al. 2016 [@moore2016critically] (and others) proposed several serious issues with BAMM, which basically boil down to two main points:

* BAMM outputs are highly sensitive to the priors used
* The likelihood function BAMM uses is wrong

Rabosky and colleagues have refuted these criticisms here: [here](http://bamm-project.org/prior.html), [here](http://bamm-project.org/replication.html), [here](http://bamm-project.org/developertoggle.html), [here](http://bamm-project.org/mea_likelihood.html), and [here](http://ift.tt/2m7qv6T).
