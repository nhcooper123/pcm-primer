...
The previous chapter introduced you to the basic building blocks of comparative methods: phylogenies and trait data. We are now going to shift gears and think about some of the theory underpinning comparative methods.
...
$observed (MSE_0 / MSE) / expected (MSE_0 / MSE)$

$MSE_0$ is the mean squared error (MSE) of the tip data in relation to the phylogenetic mean of the data. $MSE$ is the mean squared error  of the tip data in relation to the phylogenetic variance covariance matrix. We explain both of these in more detail below. The important points to take away are:
...
- the $observed (MSE_0 / MSE)$ is divided by the expected value under Brownian motion to standardise it because $observed (MSE_0 / MSE)$ will vary depending on the trait and the tree.
...
#### How do we estimate Blomberg’s *K*? Is this needed? Can it go into a box? 

Let's use a very simple example with just three taxa, A, B and C. For more detail and proper maths see [@blomberg2003phylogenetic].
...
Now let's consider just the observed $MSE_0 / MSE$ part of the equation for *K*. 
...
$MSE_0 = ((2 - 6.6)^2 + (4 - 6.6)^2 + (11 - 6.6)^2) / (3 - 1) = 23.64$
...
$MSE = ((7.219543 - 6.6)^2 + (4.902944 - 6.6)^2 + (8.633757 - 6.6)^2)/ (3 - 1) = 3.7$

As predicted, $MSE$ is much smaller than $MSE_0$ because it includes the information about the phylogeny.

We can now calculate observed $MSE_0 / MSE = 23.64/3.7 = 6.389189$ 
...
$(1/n-1) * (\Sigma V_{diag} - n/ \Sigma V^{-1})$
...
In the equation, $\Sigma V_{diag}$ is the sum of diagonal elements of $V$, i.e. $5.5 + 5.5 + 5.5 = 16.5$. $\Sigma V^{-1}$ is the sum of all elements of the inverse matrix of $V$, i.e. $0.3055556 + -0.1944444 + 0 + -0.1944444 + 0.3055556 + 0 + 0 + 0 + 0.1818182 = 0.4040404$ So our final value for expected $MSE_0 / MSE$ is $1/(3-1) * 16.5 - 3/0.4040404 = 4.5375$. 

So overall our *K* value for this simple example is

$6.389189 / 4.5375 = 1.408086$
...
$D = (d_{obs} – mean(d_{BM}))/(mean(d_{rand}) - mean(d_{BM}))$
