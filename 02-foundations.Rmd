...
Let's run a quick example simulation to see what is going on. Our trait can be any continuous feature of an organism, but for simplicity let's use body mass in cats. First let's run one simulation on have the body size of one cat lineage changes through time.
...
As above, we will start at $time = 0$ with an ancestral cat that weighs 4kg (i.e. $z(0) = 4$), and use $\sigma^2$ of 0.25 as the rate of evolution.

```{r bm100, echo = FALSE, fig.cap='Cat tree', out.width='80%', fig.asp=.75, fig.align='center'}
# Make line plots
base <-
  ggplot(sim_0.1, aes(x = time, y = trait, group = sim)) +
  labs(x = "time", y = "body size (kg)") +
  theme_cowplot() +
...
Notice how the variance in body size (i.e. the spread of different values of body size) *increases linearly with time* (it's not perfectly linear here, but it would be if we ran more simulations). Meaning that body size variance is greatest over the longest time intervals. In biological terms this means we expect to see more variance in the trait values of groups that have been evolving for longer.

```{r bmvariance, echo = FALSE, fig.cap='Cat tree', out.width='80%', fig.asp=.75, fig.align='center'}
# Make line plots
p1 <-
  base +
...
If we vary the value of $\sigma^2$ we can look at the effect of changing the rate of evolution on the values of body size we evolve in our cat simulations. If the rate of evolution is lower, then variance in body size accumulates more slowly (Figure \@ref(fig:bmplots)A); if the rate of evolution is higher, then variance in body size accumulates more quickly (Figure \@ref(fig:bmplots)C). The overall variance of the Brownian process is equalt to the rate of evolution multiplied by the amount of time that has elapsed. 

```{r bmplots, message = FALSE, echo = FALSE, fig.cap='Cat tree', out.width='80%', fig.asp=.75, fig.align='center'}
# Make line plots
plot_0.1 <-
  base + 
...
One final feature to highlight is what the body size values look like at the end of the simulations, i.e. where $t = 100$. If we assume that the present-day is $t = 100$, we can imagine that these values are equivalent to the body size values for the species at the tips of our phylogeny. 

Because trait changes are drawn from a normal distribution with a mean of 0, on average we expect no change in body size through time. So if we run enough simulations, the mean of the trait values at the end of the simulations should be equal to the starting value $z(0)$, in this case 4kg. The rest of the values should be evenly distributed around the mean, so we expect the body size values to be normally distributed. 

As demonstrated above, the variance of these normal distributions of body sizes will depend on the rate of evolution (low rates result in smaller variances; high rates result in larger variances; Figure \@ref(fig:bmhist)) *and* the time elapsed (shorter times result in smaller variances; greater times result in larger variances; Figure \@ref(fig:bmvariance)). More precisely, we can define the overall variance of the Brownian process as the rate of evolution times the amount of time that has elapsed. 
...
(plot_0.1 + ylab("") + labs(tag = "A", size = 2) + hist_0.1 )/ 
  (plot_0.25 + ylab("body size") + labs(tag = "B", size = 2) + hist_0.25) / 
...
```{r bmcat, echo = FALSE, fig.cap='Cat tree', out.width='80%', fig.asp=.75, fig.align='center'}
...
Hopefully this chapter has given you a good insight into the foundations unerlying phylogenetic comparative methods. In the next chapter we will do XXX
