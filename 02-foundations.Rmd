...
X_0.01 <- matrix(rnorm(n = nsim * (length(t) - 1), 
                       sd = sqrt(sig2_0.01)), nsim, length(t) - 1)
X_0.05 <- matrix(rnorm(n = nsim * (length(t) - 1), 
                   sd = sqrt(sig2_0.05)), nsim, length(t) - 1)
...
sim_matrix_0.01 <- cbind(rep(0, nsim), 
                         t(apply(X_0.01 , 1, cumsum)))
sim_matrix_0.05 <- cbind(rep(0, nsim), 
                         t(apply(X_0.05 , 1, cumsum)))
sim_matrix_0.1 <- cbind(rep(0, nsim), 
                        t(apply(X_0.1 , 1, cumsum)))

# Make matrices into dataframes
sim_0.01 <- melt(sim_matrix_0.01)
sim_0.05 <- melt(sim_matrix_0.05)
sim_0.1 <- melt(sim_matrix_0.1)

# Rename variables
names(sim_0.01) <- c("sim", "time", "trait")
names(sim_0.05) <- c("sim", "time", "trait")
names(sim_0.1) <- c("sim", "time", "trait")

# Make line plots
base <-
  ggplot(sim_0.1, aes(x = time, y = trait+10, group = sim)) +
...
plot_0.01 <-
  base + geom_line(data = sim_0.01)

plot_0.05 <-
  base + geom_line(data = sim_0.05)

# Now create histograms of trait values at tips
# First select only the tip values
sim_0.01_tips <- filter(sim_0.01, time == 99)
sim_0.05_tips <- filter(sim_0.05, time == 99)
sim_0.1_tips <- filter(sim_0.1, time == 99)

hist_0.01 <-
  ggplot(sim_0.01_tips, aes(x = trait+10)) +
  theme_void() +
  xlim(0,20) +
  ylim(0,35) +
  geom_histogram(bins = 12, fill = "grey", alpha = 0.8) +
  coord_flip()

hist_0.05 <-
  ggplot(sim_0.05_tips, aes(x = trait+10)) +
  theme_void() +
  xlim(0,20) +
  ylim(0,35) +
  geom_histogram(bins = 12, fill = "grey", alpha = 0.8) +
...
hist_0.1 <-
  ggplot(sim_0.1_tips, aes(x = trait+10)) +
  theme_void() +
  xlim(0,20) +
  ylim(0,35) +
  geom_histogram(bins = 12, fill = "grey", alpha = 0.8) +
...
(plot_0.1 + hist_0.1)/ (plot_0.05 + hist_0.05) / (plot_0.01 + hist_0.01)
```
```{r}
hist_0.01 <-
  ggplot(sim_0.01_tips, aes(x = trait+10)) +
...
- There is no overall drift in the direction of evolution, i.e. the direction and magnitude of the change at one time interval is independent of the direction and magnitude of the change at other time intervals.
